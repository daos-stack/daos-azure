...
load_env() {
  ...
  if [[ -f "${env_file}" ]]; then
    ...
    source "${env_file}"
    DAOS_CLIENT_ANSIBLE_COLL_URL="${DAOS_CLIENT_ANSIBLE_COLL_URL:="git+https://github.com/daos-stack/ansible-collection-daos.git"}"
    DAOS_CLIENT_ANSIBLE_PLAYBOOK="${DAOS_CLIENT_ANSIBLE_PLAYBOOK:="daos_stack.daos.azure.daos_client"}"
  else
...
install_ansible() {
  ...
  if [[ -e "${daos_coll_path}" ]]; then
    ...
    ansible-galaxy collection install "${DAOS_CLIENT_ANSIBLE_COLL_URL}"
...
install_daos() {
  ansible-playbook -c local -i '127.0.0.1,' "${DAOS_CLIENT_ANSIBLE_PLAYBOOK}" \
    --extra-vars="{
...
create_host_list_files() {
  ...
  az vmss list-instances \
    ...
    --query "[].osProfile.computerName" \
    --out tsv > /home/daos_admin/hosts_clients

  chown daos_admin:daos_admin /home/daos_admin/hosts_*
  chmod 644 /home/daos_admin/hosts_*
}
